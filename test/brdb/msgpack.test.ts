...
describe('writer deviations', () => {
  test('mpU8 (u8-typed): pfix / i8-marker / negative-fixint bands', () => {
    ...
    // 128..=223 use the I8 marker (the game casts u8 through i8 on write;
    // its field SKIPPER rejects the 0xcc uint8 form with mpack_error_type)
    expect(bytesOf(w => mpU8(w, 128))).toEqual([0xd0, 0x80]);
    expect(bytesOf(w => mpU8(w, 223))).toEqual([0xd0, 0xdf]);
    // 224..=255 (i8 -32..=-1) are single negative-fixint bytes
    expect(bytesOf(w => mpU8(w, 224))).toEqual([0xe0]);
